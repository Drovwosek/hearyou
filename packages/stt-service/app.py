...
# Добавить путь к stt-yandex
sys.path.insert(0, str(Path(__file__).parent.parent / "stt-yandex"))
...
async def process_audio_file(
    ...
    try:
        ...
        operation_id = stt.transcribe_async(
            ...
            auto_upload=True,
            hints=DEFAULT_HINTS,  # Улучшенное распознавание терминов
...
async def get_result(task_id: str):
    ...
    if status["status"] != "completed":
        raise HTTPException(
            status_code=400,
            detail=f"Task not completed yet. Status: {status['status']}"
        )
...
async def download_result(task_id: str):
    ...
    return FileResponse(
        ...
        filename=f"{status['filename']}.txt"
    )
