...
This backend sends audio to a remote transcription API when an API key is
available, while keeping the response shape close to the local Whisper provider.
"""
...
class OpusTranscriptionSTT:
    """Remote transcription backend used when an API key is configured."""
    ...
    def _request(self, audio_file: str, language: str, hints: Optional[List[str]]) -> Dict[str, Any]:
        ...
        with open(audio_file, "rb") as audio_handle:
            files = {
                "file": (Path(audio_file).name, audio_handle),
            }
